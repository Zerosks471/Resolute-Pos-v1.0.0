...
import { IconPlus, IconNotes, IconArmchair, IconScreenShare, IconSearch, IconDeviceFloppy, IconChefHat, IconCash, IconMinus, IconNote, IconTrash, IconFilter, IconPhoto, IconFilterFilled, IconClipboardList, IconX, IconClearAll, IconPencil, IconCheck, IconCarrot } from "@tabler/icons-react";
import { VITE_BACKEND_SOCKET_IO, iconStroke } from "../config/config";
import { createOrder, createOrderAndInvoice, getDrafts, initPOS, setDrafts } from "../controllers/pos.controller";
...
export default function POSPage() {
  ...
  const sendNewOrderEvent = (tokenNo, orderId) => {
    if (isSocketConnected) {
      socket.emit('new_order_backend', tokenNo, orderId);
    } else {
  ...
  if(isLoading) {
    return <Page className='px-4 py-3 flex flex-col min-h-0'>
      <div className='mt-4 h-[calc(100vh-136px)] flex gap-4 skeleton'>
        <div className='border border-resolute-border-light rounded-2xl h-full w-[70%] overflow-y-auto'></div>
        <div className='border border-resolute-border-light rounded-2xl h-full w-[30%] relative flex flex-col'></div>
      </div>
    </Page>
  }
  ...
  const btnSendToKitchen = async () => {
    try {
      ...
      toast.loading("Please wait...");
      const res = await createOrder(cartItems, deliveryType, customerType, customer, tableId);
      toast.dismiss();
      if(res.status == 200) {
        const data = res.data;
        toast.success(res.data.message);
  ...
  return (
    <Page className='px-4 py-3 flex flex-col min-h-0'>
      ...
      <div className='mt-4 md:h-[calc(100vh-136px)] flex flex-col-reverse md:flex-row gap-4'>
        ...
        {/* pos items */}
        <div className='border border-resolute-border-light rounded-2xl h-full md:w-[70%] overflow-y-auto'>
          ...
          <div className="flex gap-2 bg-white/40 backdrop-blur sticky top-0 w-full z-10 px-4 py-3 rounded-t-2xl">
            <button onClick={btnOpenCategoryFilterModal} className='bg-gray-100 hover:bg-gray-200 transition active:scale-95 text-gray-500 rounded-full w-10 h-10 flex items-center justify-center'>
              {selectedCategory != "all" ? <IconFilterFilled className='text-resolute-primary' size={18} stroke={iconStroke} /> : <IconFilter size={18} stroke={iconStroke} />}
            </button>
          ...
          {menuItems.filter((menuItem)=>{
            ...
            return <div className='bg-white border border-resolute-border-light rounded-2xl p-2 flex gap-2' key={i}>
              ...
              <div>
                ...
                <div className='mt-2'>
                    ...
                    }} className='rounded-full px-4 py-1 bg-resolute-primary hover:bg-amber-500 transition active:scale-95 text-resolute-text flex items-center justify-center'>
                      Add
                    </button>
                </div>
              </div>
        ...
        {/* cart */}
        <div className='border border-resolute-border-light rounded-2xl h-full md:w-[30%] relative flex flex-col'>
          ...
          <div className='sticky w-full px-4 py-4 bg-gradient-to-b from-white to-white/0 border-b rounded-t-2xl'>
            ...
            <div onClick={btnOpenSearchCustomerModal} className="flex items-center gap-2">
              <input value={customerType=="WALKIN"?"WALKIN CUSTOMER":`${customer.phone}-${customer.name}`} type="text" placeholder='Search Customer' className='cursor-pointer text-sm w-full border rounded-lg px-4 py-2 bg-gray-50 outline-resolute-border-light'  />
            ...
            {/* delivery type */}
            <select ref={diningOptionRef} className='mt-3 text-sm text-gray-500 w-full border rounded-lg px-4 py-2 bg-gray-50 outline-resolute-border-light'>
            ...
            {/* table selection */}
            <select ref={tableRef} className='mt-3 text-sm text-gray-500 w-full border rounded-lg px-4 py-2 bg-gray-50 outline-resolute-border-light'>
          ...
          <div className='flex-1 flex flex-col gap-4 overflow-y-auto px-4 pb-36'>
            ...
            {cartItems?.map((cartItem, i)=>{
              ...
              return <div key={i} className='text-sm border border-resolute-border-light rounded-lg p-2 relative'>
          ...
          <div className='absolute w-full bottom-0 pb-4 rounded-b-2xl px-4 bg-white/20 backdrop-blur border-t'>
            ...
            <div className="mt-2">
              <button onClick={btnShowPayAndSendToKitchenModal} className="text-sm rounded-lg border border-transparent bg-resolute-primary text-resolute-text hover:bg-amber-500 transition active:scale-95 hover:shadow-lg px-2 py-1 w-full flex items-center justify-center gap-1">
      ...
      <dialog id="modal-notes" className="modal modal-bottom sm:modal-middle">
        <div className="modal-box">
          ...
          <div className="my-4">
            ...
            <input ref={dialogNotesTextRef} type="text" name="dialogNotesText" id='dialogNotesText' className="text-sm w-full border rounded-lg px-4 py-2 bg-gray-50 outline-resolute-border-light" placeholder="Enter Notes here..." />
          </div>
          ...
          <div className="modal-action">
            <form method="dialog">
              ...
              <button onClick={()=>{btnAddNotes();}} className="rounded-lg hover:bg-amber-500 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-resolute-primary text-resolute-text ml-3">Save</button>
            </form>
          </div>
        </div>
      </dialog>
      ...
      <dialog id="modal-categories" className="modal modal-bottom sm:modal-middle">
        <div className="modal-box">
          ...
          <div className="my-4">
            ...
            <select ref={categoryFilterDropdownRef} type="text" name="select_category" id='select_category' className="text-sm w-full border rounded-lg px-4 py-2 bg-gray-50 outline-resolute-border-light" placeholder="Select Category..." >
          ...
          <div className="modal-action">
            <form method="dialog">
              ...
              <button onClick={()=>{btnApplyCategoryFilter();}} className="rounded-lg hover:bg-amber-500 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-resolute-primary text-resolute-text ml-3">Apply</button>
            </form>
          </div>
        </div>
      </dialog>
        ...
        <div className="modal-box">
          ...
          <div className="modal-action">
            <form method="dialog">
              {/* if there is a button in form, it will close the modal */}
              <button className="rounded-lg hover:bg-gray-200 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-gray-200 text-gray-500">Close</button>
              <button onClick={()=>{btnAddMenuItemToCartWithVariantsAndAddon()}} className="rounded-lg hover:bg-amber-500 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-resolute-primary text-resolute-text ml-3">Save</button>
            </form>
          </div>
        </div>
      </dialog>
      ...
      <dialog id="modal-save-draft" className="modal modal-bottom sm:modal-middle">
        <div className="modal-box">
          ...
          <div className="my-4">
            ...
            <input ref={draftTitleRef} type="text" name="draftTitleRef" id='draftTitleRef' className="text-sm w-full border rounded-lg px-4 py-2 bg-gray-50 outline-resolute-border-light" placeholder="Enter Reference Name here..." />
          </div>
          ...
          <div className="modal-action">
            <form method="dialog">
              {/* if there is a button in form, it will close the modal */}
              <button className="rounded-lg hover:bg-gray-200 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-gray-200 text-gray-500">Close</button>
              <button onClick={()=>{btnAddtoDrafts();}} className="rounded-lg hover:bg-amber-500 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-resolute-primary text-resolute-text ml-3">Save</button>
            </form>
          </div>
        </div>
      </dialog>
            ...
            {drafts.map((draftItem, index)=>{
              ...
              return <div key={index} className='flex items-center gap-1 rounded-2xl p-2 border'>
                ...
                <div className="flex flex-col gap-1">
                  <button onClick={()=>{btnSelectDraftItemToCart(draftItem)}}  className="rounded-full transition active:scale-95 text-amber-500 w-6 h-6 bg-amber-50 hover:bg-amber-500 hover:text-white flex items-center justify-center"><IconPencil size={14} stroke={iconStroke} /></button>
        ...
        <div className="modal-box">
          ...
          <div className="my-4 flex items-end gap-2">
            <div className='flex-1'>
              ...
              <input ref={searchCustomerRef} type="search" name="searchCustomerRef" id='searchCustomerRef' className="text-sm w-full border rounded-lg px-4 py-2 bg-gray-50 outline-resolute-border-light" placeholder="Enter Customer Phone here..." />
            </div>
            <button onClick={btnSearchCustomer} className="rounded-lg border bg-gray-50 hover:bg-gray-100 transition active:scale-95 hover:shadow-lg text-gray-500 flex items-center justify-center w-9 h-9">
              <IconSearch size={18} stroke={iconStroke} />
            </button>
          </div>
          ...
          <div className="modal-action">
            <form method="dialog">
              {/* if there is a button in form, it will close the modal */}
              <button onClick={btnClearSearchCustomer} className="rounded-lg hover:bg-gray-200 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-gray-200 text-gray-500">Reset</button>
              <button className="rounded-lg hover:bg-gray-200 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-gray-200 text-gray-500 ml-3">Close</button>
            </form>
          </div>
        </div>
      </dialog>
      ...
      <dialog id="modal-send-kitchen-summary" className="modal modal-bottom sm:modal-middle">
        <div className="modal-box">
          ...
          <div className="my-4 flex items-center divide-x w-full">
            ...
            <div className="flex-1 pl-4">
              <p>Payable Total</p>
              <p className="text-lg font-bold text-resolute-primary">{currency}{state.payableTotal.toFixed(2)}</p>
            </div>
          </div>
          ...
          <div className="modal-action">
            <form method="dialog">
              {/* if there is a button in form, it will close the modal */}
              <button className="rounded-lg hover:bg-gray-200 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-gray-200 text-gray-500">Close</button>
              <button onClick={btnSendToKitchen} className="rounded-lg hover:bg-amber-500 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-resolute-primary text-resolute-text ml-3">Send to Kitchen</button>
            </form>
          </div>
        </div>
      </dialog>
        ...
        <div className="modal-box">
          ...
          <div className="modal-action">
            <form method="dialog">
              {/* if there is a button in form, it will close the modal */}
              <button className="rounded-lg hover:bg-gray-200 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-gray-200 text-gray-500">Close</button>
              <button onClick={()=>{btnPayAndSendToKitchen();}} className="rounded-lg hover:bg-amber-500 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-resolute-primary text-resolute-text ml-3">Collect Payment & Send to Kitchen</button>
            </form>
          </div>
        </div>
      </dialog>
      ...
      <dialog id="modal-print-token" className="modal modal-bottom sm:modal-middle">
        <div className="modal-box">
          ...
          <div className="my-8 mx-auto w-fit">
            <div className="flex items-center gap-2">
              <div className="w-12 h-12 flex items-center justify-center bg-resolute-primary text-resolute-text rounded-full">
          ...
          <div className="modal-action justify-center">
            <form method="dialog">
              {/* if there is a button in form, it will close the modal */}
              <button className="rounded-lg hover:bg-gray-200 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-gray-200 text-gray-500">Close</button>
              <button onClick={btnPrintTokenOnly} className="rounded-lg hover:bg-amber-500 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-resolute-primary text-resolute-text ml-3">Print Token</button>
            </form>
          </div>
        </div>
      </dialog>
